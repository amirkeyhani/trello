...
from .serializers import ShortProjectSerializer, ProjectSerializer, ProjectMembershipSerializer
...
class ProjectList(ListModelMixin, CreateModelMixin, 
                  GenericAPIView):
...
class ProjectMemberList(ListModelMixin, CreateModelMixin, 
                        GenericAPIView):
...
class ProjectMemberDetail(APIView):
    ...
    def put(self, request, pk):
        ...
        serializer = ProjectMembershipSerializer(pmem, data=request.data, context={'request': request})
        if serializer.is_valid():
            serializer.save()
...
class AcceptProjectInvite(APIView):
    def post(self, request, token, format=None):
        ...
        if user is not None and ProjectMembership.objects.filter(project=project, member=user).exists() == False:
            ...
            Notification.objects.filter(
                verb='invited you to', recipient=user, target_model=ContentType.objects.get(model='project'), target_id=project.id).delete()
            return Response(status=status.HTTP_204_NO_CONTENT)
        else:
            return Response(status=status.HTTP_400_BAD_REQUEST)