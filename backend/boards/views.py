...
from .serializers import ShortBoardSerializer, BoardSerializer, ListSerializer, ItemSerializer, CommentSerializer, LabelSerializer, AttachmentSerializer, NotificationSerializer
...
class ListShow(ListCreateAPIView):
...
class ItemDetail(RetrieveUpdateDestroyAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        if 'labels' in request.data:
            ...
            if label is None:
                return Response({'labels': ['This label does not blong to this board']}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def perform_update(self, serializer):
        ...
        if 'image' in req_data:
            item = serializer.save(image_color='', color='')
        elif 'image_url' in req_data:
        ...
        if 'labels' in req_data:
            ...
            if item.labels.filter(pk=user.pk).exists():
                item.list.remove(label)
            else:
